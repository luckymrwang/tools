...
func (runner *runner) EnsureRule(position RulePosition, table Table, chain Chain, args ...string) (bool, error) {
	rp := fmt.Sprintf("%s", position)
	if strings.EqualFold(rp, "restored") {
		out, err := runner.run(operation("-t"), []string{"nat", "-S"})
		fmt.Println("restored-iptables-sart>>>>>>>>>>>>>")
		fmt.Println("restore-dout>>>", string(out))
		fmt.Println("restored-err>>>", err)
		fmt.Println("restored-iptables-end>>>>>>>>>>>>>")
		return true, nil
	}
	...
	out, err := runner.run(operation("-t"), []string{"nat", "-S"})
	fmt.Println("iptables-sart>>>>>>>>>>>>>")
	fmt.Println("out>>>\n", string(out))
	fmt.Println("err>>>", err)
	fmt.Println("iptables-end>>>>>>>>>>>>>")
	exists, err := runner.checkRule(table, chain, args...)
	fmt.Printf("checkend:[exists:%v],[err:%v]\n", exists, err)
	if err != nil {
		return false, err
	}
	if exists {
	...
	fmt.Println("runner.run>>>>>", position)
	out, err = runner.run(operation(position), fullArgs)
	fmt.Printf("runend:[out:%v],[err:%v]\n", out, err)
	if err != nil {
...
func (runner *runner) checkRuleUsingCheck(args []string) (bool, error) {
	...
	fmt.Printf("checking>>>>:[out:%v],[err:%v]\n", out, err)
